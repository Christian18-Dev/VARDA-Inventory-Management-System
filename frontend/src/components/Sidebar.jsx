...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex flex-col w-64 h-full bg-gradient-to-b bg-black text-white p-5 fixed top-0 left-0 overflow-y-auto shadow-lg z-40">
        ...
        <nav className="flex flex-col space-y-2 flex-grow">
          ...
          {Object.keys(accessibleBranches).length > 0 && (
            <>
              <button
                className="flex justify-between items-center w-full px-4 py-2 rounded-md hover:bg-red-700 transition"
                ...
                <div className="ml-4 flex flex-col space-y-2">
                  {Object.keys(accessibleBranches).map((branch) => (
                    <div key={branch} className="flex flex-col">
                      <button
                        className="flex justify-between items-center w-full px-3 py-1 rounded-md hover:bg-red-700/50 transition"
                            ...
                            <Link
                              ...
                              className={`text-sm px-3 py-1 rounded-md transition ${
                                location.pathname === store.path
                                  ...
                                  : "hover:bg-red-700/30 text-gray-200"
                              }`}
                            >
                              {store.label}
                            </Link>
          ...
          {userRole === "Admin" && (
            <SidebarLink to="/DailyTimeRecord" label="Daily Time Record" currentPath={location.pathname} 
            />
          )}
        ...
        <button
          ...
          className="mt-auto flex items-center justify-center gap-2 bg-red-700 hover:bg-red-800 text-white py-2 px-4 rounded-md transition"
        >
          <LogOut size={18} />
          Logout
        </button>
      ...
      {!isOpen && (
        <button
          className="md:hidden fixed top-2 left-2 bg-red-700 text-white p-3 rounded-md z-60 shadow-lg hover:bg-red-700 transition"
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-gradient-to-b bg-black text-white p-5 transition-transform duration-300 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:hidden z-50 shadow-lg flex flex-col`}
      >
        {/* Close Button */}
        <button
          className="absolute top-4 right-4 text-white p-2 rounded-md hover:bg-red-700 transition"
              ...
              {showInventoryDropdown && (
                <div className="ml-4 flex flex-col space-y-2">
                  {Object.keys(accessibleBranches).map((branch) => (
                    <div key={branch} className="flex flex-col">
                      <button
                        className="flex justify-between items-center w-full px-3 py-1 rounded-md hover:bg-red-400/50 transition"
                        onClick={() => toggleBranchDropdown(branch)}
                      >
                        <span>{branch}</span>
                        {expandedBranches[branch] ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                          ...
                          {accessibleBranches[branch].map((store) => (
                            <Link
                              ...
                              className={`text-sm px-3 py-1 rounded-md transition ${
                                location.pathname === store.path
                                  ? "bg-red-700 text-white"
                                  : "hover:bg-red-700 text-gray-200"
                              }`}
                            >
                              {store.label}
                            </Link>
        ...
        <div className="mt-auto pt-4">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-md transition"
          >
            <LogOut size={18} />
            Logout
          </button>
...
const SidebarLink = ({ to, label, currentPath, onClick, icon }) => (
  <Link
    to={to}
    onClick={onClick}
